...
@section('content')
    ...
    <div class="flex flex-col md:flex-row justify-between items-center  mb-6">
        <h1 class="text-2xl font-semibold">Product Tags</h1>
        <div class="flex space-x-4 mt-4 md:mt-0">
            <a href="{{ route('product-tag.create') }}" class="px-4 py-2 bg-green-500 text-white rounded hover:bg-green-600">+
                Add Product Tags</a>
        </div>
    </div>
    ...
    @push('scripts')
        <script>
            ...
            $(document).on('click', '#deleteModal .confirm-delete-btn', function() {
                ...
                const deleteUrlTemplate = "{{ route('product-tag.delete', ['slug' => '___slug___']) }}";
                $.ajax({
