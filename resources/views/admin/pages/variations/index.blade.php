...
@section('content')
    ...
    <div class="flex flex-col md:flex-row justify-between items-center  mb-6">
        <h1 class="text-2xl font-semibold">Variations</h1>
        <div class="flex space-x-4 mt-4 md:mt-0">
            <a href="{{ route('variation.create') }}" class="px-4 py-2 bg-green-500 text-white rounded hover:bg-green-600">+
                Add Variations</a>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-md p-4 w-full overflow-x-auto">
        ...
        <div id="deleteModal" class="fixed inset-0 flex items-center justify-center z-50 bg-opacity-40 hidden">
            ...
            <div class="bg-white rounded-xl w-full max-w-sm shadow-lg p-6 relative">
                <!-- Close Icon -->
                <button class="absolute cursor-pointer top-4 right-4 text-gray-500 hover:text-black text-2xl cancel-btn">
                    &times;
                </button>
                ...
                <div class="flex justify-center gap-4">
                    ...
                    <button
                        class="bg-red-600 text-white cursor-pointer font-medium px-6 py-2 rounded-md hover:bg-red-700 confirm-delete-btn">
                        Delete
                    </button>
                </div>
            </div>
        ...
        @push('scripts')
            <script>
                ...
                function changeStatus(slug) {
                    $.ajax({
                        url: "{{ route('change-variation-status') }}",
                        method: "get",
                        content: "json",
                        data: {
                            "_token": "{{ csrf_token() }}",
                ...
                $(document).on('click', '#deleteModal .confirm-delete-btn', function() {
                    ...
                    const deleteUrlTemplate = "{{ route('variation.delete', ['slug' => '___slug___']) }}";
                    $.ajax({
