...
<div class="bg-white rounded-xl shadow-md p-4 w-full overflow-x-auto">
    ...
    <input type="text" id="searchInput" placeholder="Search"
        ...
        <div class="flex gap-2">
            <select id="statusFilter" class="px-4 py-1 border border-gray-300 rounded-md text-sm">
                <option value="">Select Status</option>
                <option value="Published">Published</option>
                <option value="Draft">Draft</option>
            </select>
            <select id="sortFilter" class="px-4 py-1 border border-gray-300 rounded-md text-sm">
...
$(document).on('click', '#deleteModal .confirm-delete-btn', function() {
    ...
    $('#searchInput, #statusFilter, #sortFilter').on('input change', function() {
        table.ajax.reload();
    });
});
</script>
